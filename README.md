# ✝️ TeoLab - Teologia Cristã

> Um espaço para reflexão cristã, maturidade espiritual e formação teológica sólida — com profundidade bíblica e linguagem acessível.

---

## 📖 Sobre o projeto

**Teologia Viva** é um site de artigos e estudos teológicos que busca unir **razão e fé**, explorando temas como maturidade cristã, virtudes espirituais, vícios modernos e a restauração da imagem de Cristo em nós.

Cada texto é fruto de reflexão bíblica e teológica, com base nas Escrituras e em autores clássicos e contemporâneos, como **Timothy Keller**, **C. S. Lewis**, **Agostinho de Hipona**, etc.

---

## 🧭 Estrutura dos Artigos

Os artigos estão organizados em um formato espiritual e pedagógico, guiando o leitor por temas fundamentais da vida cristã:

- 💭 **Propedêutica Teológica**
- 🪞 **Maturidade Cristã**
- 🧩 **Antropologia Bíblica** 
- 🕊️ **Apologética Cristã**
- 🔥 **Estudos Bíblicos** 
- ❤️‍🩹 **História da Igreja**

---

## 🛠️ Tecnologias Utilizadas

O projeto foi desenvolvido com tecnologias modernas de frontend, garantindo performance e uma experiência de leitura fluida:

| Tecnologia | Função |
|-------------|---------|
| ⚛️ **React + TypeScript** | Base da aplicação |
| 🧭 **React Router DOM** | Navegação entre artigos e seções |
| 💅 **Tailwind CSS** | Estilização rápida e responsiva |
| 🧩 **Shadcn/UI** | Componentes acessíveis e consistentes |
| 🕊️ **Lucide Icons** | Ícones leves e elegantes |
| 📦 **Vite** | Build e dev server de alta performance |

---